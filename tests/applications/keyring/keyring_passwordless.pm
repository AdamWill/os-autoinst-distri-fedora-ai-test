...
sub export_kde_vars {
    ...
    # Set them now.
    enter_cmd('export SSH_ASKPASS=/usr/bin/ksshaskpass', max_interval => 1);
    sleep 2;
    enter_cmd('export SSH_ASKPASS_REQUIRE=prefer', max_interval => 1);
    sleep 2;
}
...
sub connect_localhost {
    ...
    enter_cmd($command, max_interval => 1);
    ...
    enter_cmd("bye", max_interval => 1);
    # Exit the terminal app.
    enter_cmd("exit", max_interval => 1);
}
